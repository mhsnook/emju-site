...
//
// The bundle is report-only to start with. There is no baseline for this
// project yet, so any budget would be a guess. Once a few PRs have shown what
// ordinary growth looks like, replace it with `{ maxGzDelta: '20KiB' }` — the
// sidecar already carries `eagerGzDelta` and `eagerGzBase` for that.
const POLICY = {
	build: 'no-new',
	typecheck: 'no-new',
	lint: 'no-new',
...
// job passes the gate.
//
// `bundle` is not here: it is report-only, and a bundle measurement is missing
// on any PR whose build failed — which the build check already blocks on.
