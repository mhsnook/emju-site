...
// The first line of an error in Vite's output. Rolldown prints
// `error during build:` or `[vite]: Rollup failed to resolve …`, and a plugin
// failure prints `✘` or `×`. The excerpt is anchored on this rather than on the
// tail of the log, because Vite prints a stack and an exit code after the
// useful part.
